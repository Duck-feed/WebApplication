...
export default function HorizontalScroll({ children, className, onDragStateChange }: Props) {
  ...
  const updateConstraints = useCallback(() => {
    ...
    timeoutRef.current = setTimeout(() => {
      ...
      setConstraints(maxDrag > 0 ? { left: -maxDrag, right: 0 } : { left: 0, right: 0 });
    }, 100);
  }, []);
  ...
  return (
    ...
    >
      <motion.div
        ref={contentRef}
        className={`flex gap-2 ${canDrag ? "cursor-grab active:cursor-grabbing" : ""}`}
